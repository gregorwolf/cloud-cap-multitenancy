...
// - process.env must conatin CF_API, CF_API_USER
// - VCAP_SERVICES (with name services) and VCAP_APPLICATION (with name application) must exist in ./handlers/vcap-local.json
// - default-env.json must exist
  ...
  try {
    ...
    try {
      ...
      let options2 = {
        method: "POST",
        url:
          res1.data.authorization_endpoint + "/oauth/token?grant_type=password",
        headers: {
  ...
  try {
    ...
    try {
      let options2 = {
        method: "POST",
        url:
          appEnv.app.cf_api + "/v3/routes/" + res1.data.guid + "/destinations",
        headers: {
          Authorization: "Bearer " + connectRes.access_token,
          "Content-Type": "application/json",
        },
  ...
  try {
    ...
    try {
      ...
      let options1 = {
        method: "GET",
        url:
          services.registry.saas_registry_url +
          "/saas-manager/v1/application/subscriptions?tenantId=" +
          tenantID,
        headers: {
      ...
      try {
        ...
        try {
          let options3 = {
            method: "DELETE",
            url:
              appEnv.app.cf_api + "/v3/routes/" + res2.data.resources[0].guid,
            headers: {
              Authorization: "Bearer " + connectRes.access_token,
            },
          };
  ...
  try {
    ...
    try {
      ...
      let options2 = {
        ...
        url:
          services.sm.sm_url +
          "/v1/service_bindings/?labelQuery=tenant_id+eq+" +
          "'" +
          tenantID +
          "'",
        headers: {
...
module.exports = (service) => {
  ...
  service.on("UPDATE", "tenant", async (req, next) => {
    console.log(
      "[INFO][ON_UPDATE_TENANT] XXX_Starting On Subscription for " +
      ...
      function (res0) {
        ...
        doDeploy(pwd + "/hdbspecific", hdienv, logger).then(
          function (resx) {
            ...
            //         console.log(
            //           "[INFO][ON_UPDATE_TENANT] XXX_Ending On Subscription for " +
            //             req.data.subscribedSubdomain +
            //             "."
            //         );
  ...
  service.before("DELETE", "tenant", async (req, next) => {
    ...
    console.log(
      "[INFO ][ON_DELETE_TENANT] XXX_Before Unsubscription for " +
  ...
  service.on("DELETE", "tenant", async (req, next) => {
    console.log("delete req:" + inspect(req.data, false, 1));
    console.log(
      "[INFO ][ON_DELETE_TENANT] XXX_On Unsubscription for " +
        req.data.subscribedTenantId +
        "."
    );
